...
sample = 2   #number of samples in avg calc
...
#compute "sample" readings avg of p_delta here eventually
    if count>=(sample):
        ...
        #compute angle by scaling avg_p_delta to 1 radian or pi
        ...
        # Radial display loop
        angle=((avg_p_delta*pi())/pi())
        length=20
        #for angle in range(0, 360, 5):
        display.fill(0)  # Clear the display
        draw_needle(angle, length, 1)  # Draw the needle
        display.show()
        time.sleep(0.1)
